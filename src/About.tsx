...
export default function About() {
  ...
  return (
    <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col md:flex-row gap-6">
      ...
      <nav className={`hidden md:flex w-52 flex-shrink-0 rounded-2xl p-3 flex-col gap-1 self-start ${card}`}>
        ...
        {tabs.map(tab => (
          <button
            ...
            className={`flex items-center gap-3 px-3 py-3 rounded-xl text-left text-sm font-semibold transition-all duration-200 cursor-pointer ${activeTab === tab.id ? tabActive : tabIdle}`}
          >
            <span>{tab.icon}</span>
            {tab.label}
          </button>
      ...
      <div className="flex-1 relative w-full flex flex-col gap-6 md:gap-4 pb-4">
